...
export default function Dialog({ onVerified, onCancel, user }) {
  ...
  async function getChallenge(event) {
    ...
    const result = await fetch(
      `/api/attestations/youtube?username=${username.replace('@', '')}`
    )
    if (!result.ok) {
      setWaiting(false)
  ...
  async function verify(event) {
    ...
    result = await fetch('/api/attestations/youtube', {
      method: 'POST',
    ...
    <>
      ...
      {challenge ? (
        <form onSubmit={verify}>
          ...
          <p>
            Add the following challenge in your YouTube channel's description.
            When ready click attest to verify it. Once attested you can delete it.
          </p>
          <pre>{challenge}</pre>
          <Link href={`https://youtube.com/${username}/about`} target="_blank">
        ...
        <form onSubmit={getChallenge}>
          <h4>Set YouTube Challenge</h4>
          <p>
            Enter your YouTube handle to recieve a unique code to confirm
